...
import org.opensearch.commons.notifications.model.Feature.IndexManagement
...
class QueryNotificationConfigIT : PluginRestTestCase() {
    ...
    private fun createConfig(
        ...
        features: Set<Feature> = setOf(Alerting, IndexManagement, Feature.Reports)
    ): String {
    ...
    fun `test Get sorted notification config using single keyword sort_field(config_type)`() {
        ...
        val emailGroupId = createConfig(configType = ConfigType.EmailGroup)
        val smtpAccountId = createConfig(configType = ConfigType.SmtpAccount)
        Thread.sleep(1000)
    ...
    fun `test Get sorted notification config using multi keyword sort_field(features)`() {
        val iId = createConfig(features = setOf(IndexManagement))
        val aId = createConfig(features = setOf(Alerting))
        val rId = createConfig(features = setOf(Reports))
        val iaId = createConfig(features = setOf(IndexManagement, Alerting))
        val raId = createConfig(features = setOf(Reports, Alerting))
        val riId = createConfig(features = setOf(Reports, IndexManagement))
        val iarId = createConfig(features = setOf(IndexManagement, Alerting, Reports))
        Thread.sleep(1000)
        ...
        val getDefaultOrderConfigResponse = executeRequest(
            RestRequest.Method.GET.name,
            "$PLUGIN_BASE_URI/configs?sort_field=features",
            "",
            RestStatus.OK.status
        )
        verifyOrderedConfigList(sortedConfigIds, getDefaultOrderConfigResponse, sortedConfigIds.size)
        ...
        val getAscConfigResponse = executeRequest(
            RestRequest.Method.GET.name,
            "$PLUGIN_BASE_URI/configs?sort_field=features&sort_order=asc",
            "",
            RestStatus.OK.status
        )
        verifyOrderedConfigList(sortedConfigIds, getAscConfigResponse, sortedConfigIds.size)
        ...
        val getDescConfigResponse = executeRequest(
            RestRequest.Method.GET.name,
            "$PLUGIN_BASE_URI/configs?sort_field=features&sort_order=desc",
            "",
            RestStatus.OK.status
        )
    ...
    fun `test Get filtered notification config using keyword filter_params(config_type)`() {
        val slackId = createConfig(configType = ConfigType.Slack)
        val chimeId = createConfig(configType = ConfigType.Chime)
        val webhookId = createConfig(configType = ConfigType.Webhook)
        val emailGroupId = createConfig(configType = ConfigType.EmailGroup)
        ...
        val getWebhookEmailGroupOrSmtpAccountResponse = executeRequest(
            RestRequest.Method.GET.name,
            "$PLUGIN_BASE_URI/configs?config_type=Webhook,EmailGroup,SmtpAccount",
            "",
            RestStatus.OK.status
        )
    ...
    fun `test Get filtered notification config using text filter_params(name)`() {
    ...
    fun `test Get filtered notification config using keyword filter_params(is_enabled)`() {
    ...
    fun `test Get filtered notification config using keyword filter_params(features)`() {
        val iId = createConfig(features = setOf(IndexManagement))
        val aId = createConfig(features = setOf(Alerting))
        val rId = createConfig(features = setOf(Reports))
        val iaId = createConfig(features = setOf(IndexManagement, Alerting))
        ...
        val getEnabledResponse = executeRequest(
            RestRequest.Method.GET.name,
            "$PLUGIN_BASE_URI/configs?features=Reports",
            "",
            RestStatus.OK.status
        )
        ...
        val getDisabledResponse = executeRequest(
            RestRequest.Method.GET.name,
            "$PLUGIN_BASE_URI/configs?features=IndexManagement,Alerting",
            "",
            RestStatus.OK.status
        )
